...
def create_app() -> FastAPI:
    ...
    async def run_tool(req: ToolRunRequest):
        tool = _tool_instances.get(req.tool)
        if not tool:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Grok Bridge Tool Server")
    ...
    allowed = [os.path.dirname(os.path.dirname(os.path.abspath(__file__)))]
